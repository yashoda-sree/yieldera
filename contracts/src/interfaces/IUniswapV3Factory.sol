...
interface IUniswapV3Factory {
    ...
    /// @dev Can be changed by the current rentPayer via setRentPayer
    /// @return The address of the rentPayer
    function rentPayer() external view returns (address);

    /// @notice Returns current pool create fee
    /// @dev this can be changed by the current owner via setPoolCreateFee
    /// @return current pool create fee in tiny cents
    ...
    /// @notice Returns the pool address for a given pool of tokens and a fee, or address 0 if it does not exist
    ...
    /// @notice sets the pool create fee in tinybars to deter malicious state expansion
    /// @param _poolCreateFee new pool create fee
    ...
    /// @dev Must be called by the rentPayer
    /// @param _rentPayer The new rentPayer of the factory
